...
def add_reading_to_db(time, amplitude, lat, long, alt=0.0):
    ...
    c.execute("INSERT INTO Sensors (X, Y, Z, T, Amplitude) VALUES (?, ?, ?, ?, ?);", (x, y, z, time_to_str(query_to_time(time)), amplitude))
    conn.commit()
...
def add_event_to_db(time, x, y, z):
    ...
    c.execute("INSERT INTO Events (X, Y, Z, T) VALUES (?, ?, ?);", (lat, lon, time_to_str(t)))
    conn.commit()
...
def dump_for_delay(conn=conn):
    ...
    for row in c.execute("SELECT X, Y, Z, T FROM Sensors;"):
        row_conv = (row[0], row[1], row[2], (str_to_time(row[3]) - datetime(2019, 1, 1, tzinfo=timezone.utc)) / timedelta(seconds=1) * V_Sound)
        if not t_first:
